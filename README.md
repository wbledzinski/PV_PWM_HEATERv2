...
Using PV array to produce heat sounds as not particularly bright idea: during wintertime there is no enough sunny weather to produce reasonable amount of energy from PV array to heat whole building (I live at 50* latitude in Europe).
...
It is basic idea of RC oscillator. The only difference is that PV array of that size produces 360VDC (disconnected even 440VDC) and almost 10A continuosly: RC oscillators usually have much less power.
...
Critical components are big cap and two power transistors. I tested Infineons SiC parts IMZ120R030M1HXKSA1 and IMZ120R045M1XKSA1 toogether with EPCOS B32674D6475K000 4u7 and B32774D8126K000 12uF (don't be suprised, big cap value gives acoustic noise due to low frequency of operation BUT low frequency generates less commuting loses).
Actually I don't care about noise generated by caps, my controller and heat tank is installed in basement. So I've hosen lower frequency to decrease switching loses.
...
3. to prevent pressure build-up (in case of NTC failure or transistor malfunction ) that tank itself has an overflow port - it allows to escape if there is any excessive prese, no matter the cause: from excessive water level or from steam
4. to prevent any burn due to very hot tap vater I installed hydraulic thermostat valve at buffer output, it is set to 55C
5. to prevent any bacteria to growth you can't store 1000l of tap water. Buffer has built-in heat exchanger in form of spiral copper pipe (copper itself is bacteriostatic), 20mm diameter, 50m long. Heat exchanger capacity is about 10liters. 1000l of water accumulates heat and never will be used for consumtion directly in any form.
...
the only damage to first version (100% my fault, just don't short drain and source during operation, it will create an arc)

![schema](/Photo/dcarc.png)
